...
export const EMPLOYEE_PAGES = {
  office: {
    home: {
      ...
      description: tx("오피스형 직원은 출퇴근보다 요청, 서명, 일정 확인을 먼저 처리해야 합니다.", "Office employees typically need to handle requests, signatures, and schedule checks before anything else."),
      questions: [
        tx("오늘 제출하거나 확인해야 할 요청이 무엇인가", "What requests do I need to submit or review today"),
      ...
      weekly: [
        tx("수요일 재택", "Remote on Wednesday"),
        tx("금요일 연차 예정", "Leave planned on Friday"),
        tx("1:1 미팅 14:00", "1:1 meeting at 14:00"),
      ],
      summary: [
        tx("잔여 연차 8.5일", "8.5 leave days left"),
    ...
    requests: {
      ...
      description: tx("오피스형 요청 허브는 휴가, 일반 품의, 문서 관련 요청과 반려 재제출을 빠르게 처리해야 합니다.", "The office request hub should help employees quickly handle leave, general approvals, document-related requests, and rejected resubmissions."),
      quickActions: [
        tx("연차 요청", "Leave request"),
      ...
      history: [
        tx("연차 요청 / 승인 대기", "Leave request / awaiting approval"),
    ...
    signatures: {
      ...
      description: tx("오피스형 서명 인박스는 계약, 정책 동의, 문서 재발송 대기건을 먼저 처리해야 합니다.", "The office signature inbox should prioritize contracts, policy acknowledgements, and resend queues."),
      priority: [
  ...
  retail: {
    home: {
      ...
      description: tx("리테일형 직원은 오늘 근무, 체크인 상태, 시프트 변경과 공지를 먼저 확인해야 합니다.", "Retail employees should prioritize today's shift, check-in state, shift changes, and field notices."),
      questions: [
        tx("오늘 어느 매장에서 몇 시에 근무하는가", "Where and when am I working today"),
        tx("체크인 전에 확인할 공지나 변경이 있는가", "Which notices or changes need review before check-in"),
        tx("대체 근무나 추가 호출이 들어왔는가", "Whether I received a swap or call-in request"),
      ],
      mustDo: [
        tx("11:30 오픈조 체크인", "Check in for 11:30 opening shift"),
        tx("브레이크 규정 개정 확인", "Review revised break policy"),
        tx("대체 근무 요청 응답", "Respond to shift swap request"),
      ],
      quickActions: [
        tx("체크인", "Check in"),
        tx("체크아웃", "Check out"),
        tx("시프트 확인", "View shift"),
        tx("대체 근무 응답", "Respond to swap"),
      ],
      weekly: [
    ...
    requests: {
      ...
      description: tx("리테일형 요청 허브는 출퇴근 정정, 시프트 변경 응답, 대체 근무 회신을 빠르게 처리해야 합니다.", "The retail request hub should help employees quickly handle attendance corrections, shift changes, and swap responses."),
      quickActions: [
        tx("출퇴근 정정", "Attendance correction"),
        tx("시프트 변경 응답", "Shift change response"),
        tx("대체 근무 회신", "Swap response"),
        tx("근무 문의", "Work request"),
      ],
      hints: [
        tx("브레이크 규정 위반은 매장장 확인 필요", "Break-rule exceptions need store manager review"),
        tx("초과근무 요청은 당일 승인 기준", "Overtime requests are reviewed same day"),
      ],
      history: [
        tx("체크아웃 정정 / 승인 대기", "Checkout correction / awaiting approval"),
        tx("대체 근무 회신 / 확인 필요", "Swap response / needs confirmation"),
        tx("시프트 변경 / 승인 완료", "Shift change / approved"),
      ],
    },
    signatures: {
      title: tx("Retail Pack Signature Inbox", "Retail Pack Signature Inbox"),
      description: tx("리테일형 서명 인박스는 매장 운영 규정, 현장 공지, 근무 관련 동의 문서를 먼저 처리해야 합니다.", "The retail signature inbox should prioritize store policy, field notices, and work-related acknowledgement documents."),
      priority: [
        tx("매장 운영 규정 / 오늘 마감", "Store policy / due today"),
        tx("현장 공지 동의 / 내일 오픈 전 확인", "Field notice acknowledgement / review before tomorrow open"),
      ],
      alerts: [
        tx("대체 근무 안내 확인", "Review swap guidance"),
