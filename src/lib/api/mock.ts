    ...
    columns: [
      {
        ...
        items: tenants
          ...
          .map((tenant) => ({
            ...
            meta:
              tenant.pack === "office"
                ? tx("갱신 · 승인 흐름 점검", "Review renewal and approval flow")
                : tx("커버리지 · 결제 리스크 점검", "Review coverage and billing risk"),
      ...
      },
      {
        ...
        items: [
          ...
          { title: tx("비정상 로그인 시도 4건", "4 suspicious login attempts"), meta: tx("IP와 tenant 영향 확인", "Review IPs and impacted tenants"), tone: "watch" },
          { title: tx("지원 SLA 지연 1건", "1 delayed support SLA"), meta: tx("오늘 처리", "Handle today"), tone: "critical" },
        ],
      },
    ],
      ...
      view === "settings"
        ...
        : view === "workflow" && approvalEvents.length
          ? {
              ...
              description: tx(
                "현재 tenant에서 실행한 승인 액션이 최근 기록으로 노출됩니다.",
