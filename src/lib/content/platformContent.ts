...
export const PLATFORM_PAGE = {
  ...
  description: tx(
    "고객사 HR 운영 화면과 분리된 백오피스입니다. 운영 리스크, 과금, 지원, 보안, 감사 흐름을 한 곳에서 처리합니다.",
  ...
  questions: [
    ...
    tx("지금 즉시 조치해야 할 시스템 신호가 있는가", "Which system signals require action now"),
  ],
  kpis: [
    { value: "124", label: tx("활성 테넌트", "Active tenants") },
    { value: "7", label: tx("유예 계정", "Grace accounts") },
  ...
  columns: [
    {
      ...
      items: [
        ...
        { title: tx("장애 영향 추적 테넌트 2건", "2 tenants under incident watch"), meta: tx("지원", "Support"), tone: "watch" },
    ...
    },
    {
      ...
      items: [
        { title: tx("API 성공률 99.92%", "API success rate 99.92%"), meta: tx("정상 / 추가 조치 불필요", "Healthy / No action needed"), tone: "healthy" },
        { title: tx("Webhook 전달 성공률 94.10%", "Webhook delivery 94.10%"), meta: tx("주의 / 재시도 큐 확인", "Watch / Review retry queue"), tone: "watch" },
        { title: tx("SSO 로그인 오류 18건", "18 SSO login errors"), meta: tx("주의 / 영향 테넌트 확인", "Watch / Check tenant impact"), tone: "watch" },
    ...
    },
    {
      ...
      items: [
        { title: tx("Acme Corp / 체험에서 활성로 전환", "Acme Corp / Trial to Active"), meta: tx("플랜 전환", "Plan transition") },
        { title: tx("Nova Team / 활성에서 유예로 변경", "Nova Team / Active to Grace"), meta: tx("결제 추적", "Billing follow-up") },
